...
class exchange:
    ...
    def buy(self, amount, price,tradePassword=None,tradeid=None):
        ...
        if self.role == 'huobi':
            ...
            r = requests.post(self.url['host'], params=payload)
    ...
    def marketBuy(self, amount,tradePassword=None,tradeid=None):
        if amount <=0 :
            return
    ...
    def marketSell(self, amount, tradePassword=None,tradeid=None):
        if amount <=0 :
            return
    ...
    def batchTrade(self, arr):
        if self.role == 'haobtc' or self.role == 'default':
            payload = {'api_key':self.apikey,'secret_key':self.secretToken,'orders_data':batchTradeFormat(arr)}
            payload = tradeLoad(payload, self.secretToken, self.role)
            return requestPost(self.url['batch_trade'], payload)

        if self.role == 'okcoin':
            body = requestBody(self.url['batch_trade'], self.url['host'])
            params = {
                'api_key':self.apikey,
                'symbol':'btc_cny',
                'type':'',
    ...
    def historyInfo(self, size=5, orderID=0):
        if self.role == 'haobtc' or self.role == 'default':
            if orderID ==0:
                payload = {'api_key':self.apikey,'size':size}
            else:
                payload = {'api_key':self.apikey,'size':1,'order_id':orderID}
    ...
    def ledger(self,size):
        if self.role == 'haobtc' or self.role == 'default':
            payload = {'api_key':self.apikey,'size':size}
            payload = tradeLoad(payload, self.secretToken , self.role)
            return requestPost(self.url['ledger'] , payload)
    ...
    def hedge(self ,robot,  pos = 0, size=1000 ):
        if self.role == 'haobtc' or self.role == 'default':
            if pos == 0:
                payload = {'api_key':self.apikey,'size':size,'ids':batchTradeFormat(robot)}
            else:
                payload = {'api_key':self.apikey,'size':size,'pos':pos,'ids':batchTradeFormat(robot)}

            payload = tradeLoad(payload, self.secretToken , self.role)
            s=requestPost(self.url['hedge'], payload)
            return s
    ...
    def ticker(self,symbol=''):
        ...
        if self.role == 'btcc':
            payload = {'market':'btccny'}
            return requestGet(self.url['ticker'], payload)
